...
class PaymentCallbackAuthenticatorTest {
    ...
    private final PaymentCallbackAuthenticator authenticator = new PaymentCallbackAuthenticator(
            SECRET,
    ...
    void acceptsAnAuthenticRecentPayload() {
        ...
        assertThatCode(() -> authenticator.verify(
                timestamp, authenticator.signature(timestamp, payload), payload
    ...
    void rejectsTamperingAndStaleReplayAttempts() {
        ...
        assertThatThrownBy(() -> authenticator.verify(timestamp, signature, "tampered"))
                .isInstanceOf(PaymentCallbackAuthenticationException.class)
                .hasMessageContaining("signature");

        String stale = String.valueOf(NOW.minus(Duration.ofMinutes(6)).getEpochSecond());
        assertThatThrownBy(() -> authenticator.verify(
                stale, authenticator.signature(stale, "original"), "original"
        )).isInstanceOf(PaymentCallbackAuthenticationException.class)
