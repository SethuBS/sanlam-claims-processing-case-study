...
class ClaimServiceTest {
    ...
    void validStraightThroughClaimCreatesOnePaymentAndBecomesPending() {
        ...
        when(clientRegistryClient.validate(claim.getClientId())).thenReturn(
                new ClientRegistryClient.ClientValidationResult("CV-1", true, "ACTIVE", new String[0])
        );
        when(policyManagerClient.check(claim.getPolicyNumber(), claim.getAmount())).thenReturn(
                new PolicyManagerClient.EligibilityResult("PV-1", true, false, "v1", new String[0])
        );
        when(paymentSystemClient.createPayment(claim.getId(), claim.getAmount(), claim.getCurrency()))
                .thenReturn(new PaymentSystemClient.PaymentAccepted("PAY-001", "ACCEPTED"));
    ...
    void manualReviewStopsBeforePayment() {
        ...
        when(policyManagerClient.check(claim.getPolicyNumber(), claim.getAmount())).thenReturn(
                new PolicyManagerClient.EligibilityResult("PV-1", true, true, "v1", new String[0])
        );
    ...
    void invalidClientIsBusinessRejectionAndSkipsOtherSystems() {
        ...
        when(claimRepository.findById(claim.getId())).thenReturn(Optional.of(claim));
        when(clientRegistryClient.validate(claim.getClientId())).thenReturn(
                new ClientRegistryClient.ClientValidationResult(
                        "CV-1", false, "INACTIVE", new String[]{"CLIENT_NOT_ACTIVE"}
                )
        );
