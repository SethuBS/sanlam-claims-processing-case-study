package com.sanlam.claims.contract;
...
class OpenApiContractTest
{
    ...
    void parsesAndContainsTheRequiredOperation(String resource, String path, String responseCode)
    {
        ...
        SwaggerParseResult result = new OpenAPIParser().readLocation(url.toExternalForm(), null, new ParseOptions());
    ...
    {
        return Stream.of(Arguments.of("client-registry.yaml", "/api/v1/client-validations", "200"),
    ...
    void claimsApiContractMatchesTheImplementedEndpoints()
    {
        ...
        assertThat(result.getMessages()).isEmpty();
        OpenAPI api = result.getOpenAPI();
        assertThat(api.getPaths()).containsKeys("/api/v1/claims", "/api/v1/claims/{claimId}",
                "/api/v1/claims/{claimId}/decisions/approve", "/api/v1/claims/{claimId}/decisions/reject",
                "/internal/v1/payment-status-events");
        assertThat(api.getPaths().get("/api/v1/claims").getPost().getResponses()).containsKey("202");
        assertThat(api.getPaths().get("/internal/v1/payment-status-events").getPost().getParameters())
