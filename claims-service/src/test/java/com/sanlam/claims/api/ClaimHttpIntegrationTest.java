...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = "payment.reconciliation.enabled=false")
...
class ClaimHttpIntegrationTest extends PostgresIntegrationSupport
{
    ...
    @Autowired
    PaymentCallbackAuthenticator callbackAuthenticator;
    ...
    {
        given().contentType("application/json").header("Idempotency-Key", "http-key-1").body(validRequest("WEB-HTTP-1"))
                .when().post("/api/v1/claims").then().statusCode(HttpStatus.ACCEPTED.value())
                .header("Location", notNullValue()).body("claimId", notNullValue())
                .body("externalReference", equalTo("WEB-HTTP-1")).body("status", equalTo("RECEIVED"))
    ...
    {
        given().contentType("application/json").header("Idempotency-Key", "http-key-invalid")
                .body(validRequest("WEB-HTTP-INVALID").replace("250000.00", "0")).when().post("/api/v1/claims").then()
    ...
    {
        given().contentType("application/json").header("Idempotency-Key", "http-key-conflict")
                .body(validRequest("WEB-HTTP-A")).post("/api/v1/claims").then().statusCode(HttpStatus.ACCEPTED.value());

        given().contentType("application/json").header("Idempotency-Key", "http-key-conflict")
                .body(validRequest("WEB-HTTP-B")).when().post("/api/v1/claims").then()
                .statusCode(HttpStatus.CONFLICT.value()).contentType("application/problem+json")
    ...
    {
        given().when().get("/api/v1/claims/{claimId}", UUID.randomUUID()).then()
                .statusCode(HttpStatus.NOT_FOUND.value()).body("code", equalTo("CLAIM_NOT_FOUND"));
    }

    @Test
    void internalPaymentCallbackRequiresServiceAuthentication()
    {
        given().contentType("application/json").body("{}").when().post("/internal/v1/payment-status-events").then()
                .statusCode(HttpStatus.UNAUTHORIZED.value()).body("code", equalTo("PAYMENT_CALLBACK_UNAUTHENTICATED"));
    }

    @Test
    void authenticPaymentCallbackIsAcceptedAndApplied()
    {
        Claim claim = Claim.receive(UUID.randomUUID(), "WEB-SIGNED-CALLBACK", "CLIENT-1", "POL-1", ClaimType.STANDARD,
                LocalDate.of(2026, 8, 31), BigDecimal.TEN, "ZAR", Instant.now());
        ...
        given().contentType("application/json").header("X-Callback-Timestamp", timestamp)
                .header("X-Callback-Signature", callbackAuthenticator.signature(timestamp, payload)).body(payload)
                .when().post("/internal/v1/payment-status-events").then().statusCode(HttpStatus.ACCEPTED.value());

        org.assertj.core.api.Assertions.assertThat(claimRepository.findById(claim.getId()).orElseThrow().getStatus())
