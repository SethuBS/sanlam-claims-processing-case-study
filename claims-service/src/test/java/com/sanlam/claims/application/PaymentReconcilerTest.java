...
class PaymentReconcilerTest
{
    ...
    void setUp()
    {
        reconciler = new PaymentReconciler(claimRepository, paymentSystemClient, Clock.fixed(NOW, ZoneOffset.UTC),
    ...
    void recoversACompletedPaymentWhenTheCallbackWasLost()
    {
        ...
        when(claimRepository.findTop20ByStatusAndUpdatedAtBeforeOrderByUpdatedAtAsc(ClaimStatus.PAYMENT_PENDING,
                NOW.minusSeconds(5))).thenReturn(List.of(claim));
        when(paymentSystemClient.getPayment(claim.getId()))
                .thenReturn(new PaymentResponse("PAY-RECOVERED", "COMPLETED"));
    ...
    void downstreamOutageLeavesTheClaimPendingForTheNextPass()
    {
        ...
        when(paymentSystemClient.getPayment(claim.getId())).thenThrow(new IllegalStateException("temporary outage"));
    ...
    {
        Claim claim = Claim.receive(UUID.randomUUID(), "WEB-RECON", "CLIENT-1", "POL-1", ClaimType.STANDARD,
