...
public class PaymentReconciler
{
    ...
    public PaymentReconciler(ClaimRepository claimRepository, PaymentSystemClient paymentSystemClient, Clock clock,
    ...
    public void reconcile()
    {
        var candidates = claimRepository.findTop20ByStatusAndUpdatedAtBeforeOrderByUpdatedAtAsc(
                ClaimStatus.PAYMENT_PENDING, clock.instant().minus(minimumAge));

        for (Claim claim : candidates)
        {
            try
            {
                PaymentResponse payment = paymentSystemClient.getPayment(claim.getId());
                if (payment == null)
                {
                    continue;
                }
            ...
            {
                // A failed lookup stays pending and will be retried on the next reconciliation
                // pass.
            }
        }
    }
