...
public class ClaimService
{
    ...
    public ClaimService(ClaimRepository claimRepository, IdempotencyRecordRepository idempotencyRecordRepository,
            OutboxEventRepository outboxEventRepository,
            ProcessedPaymentEventRepository processedPaymentEventRepository, ClientRegistryClient clientRegistryClient,
            PolicyManagerClient policyManagerClient, PaymentSystemClient paymentSystemClient, ObjectMapper objectMapper,
            Clock clock)
    {
    ...
    public Claim submit(ClaimCommands.SubmitClaim command)
    {
        ...
        try
        {
            ...
            Claim claim = Claim.receive(UUID.randomUUID(), command.externalReference(), command.clientId(),
                    command.policyNumber(), command.claimType(), command.incidentDate(), command.amount(),
                    command.currency(), now);

            claimRepository.save(claim);
            idempotencyRecordRepository.save(new IdempotencyRecord(UUID.randomUUID(), command.idempotencyKey(),
                    requestHash, claim.getId(), now));
            outboxEventRepository
                    .save(new OutboxEvent(UUID.randomUUID(), "claim.submitted", claim.getId(), payload(claim), now));
        ...
        catch (DataIntegrityViolationException exception)
        {
            return idempotencyRecordRepository.findByIdempotencyKey(command.idempotencyKey())
                    .flatMap(record -> claimRepository.findById(record.getClaimId())).orElseThrow(() -> exception);
        }
    }
    ...
    public Claim process(UUID claimId)
    {
        ...
        var eligibility = policyManagerClient.check(claim.getPolicyNumber(), claim.getAmount());
        ...
        PaymentResponse payment = paymentSystemClient.createPayment(claim.getId(), claim.getAmount(),
                claim.getCurrency());
    ...
    @Transactional
    public boolean handlePaymentStatusEvent(UUID eventId, UUID claimId, String paymentReference, String status)
    {
        int registered = processedPaymentEventRepository.registerIfNew(eventId, claimId, clock.instant());
        if (registered == 0)
        {
            return false;
        }
    ...
    private String payload(Claim claim)
    {
        try
        {
            return objectMapper.writeValueAsString(new ClaimSubmittedPayload(claim.getId(),
                    claim.getExternalReference(), claim.getClaimType().name()));
        }
    ...
    private record ClaimSubmittedPayload(UUID claimId, String externalReference, String claimType)
    {
    }
}
