...
import com.sanlam.mock.payment.dto.request.PaymentStatusEventRequest;
...
public class PaymentController
{
    ...
    public PaymentController(RestClient.Builder builder, ObjectMapper objectMapper,
            ...
            @RequestBody PaymentRequest request)
    {
        PaymentRecord payment = paymentsByKey.computeIfAbsent(idempotencyKey, key -> {
            PaymentRecord created = new PaymentRecord(request.claimId(), "PAY-" + UUID.randomUUID(), UUID.randomUUID());
        ...
        switch (behaviour)
        {
            ...
            case "TEMPORARY_FAILURE" ->
                throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "Temporary payment service failure");
            ...
            default -> throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Unsupported payment behaviour: " + behaviour);
        }
    ...
    private void scheduleCallback(PaymentRecord payment, long delayMs)
    {
        CompletableFuture.delayedExecutor(delayMs, TimeUnit.MILLISECONDS).execute(() -> {
            try
            {
                PaymentStatusEventRequest event = new PaymentStatusEventRequest(payment.eventId, payment.claimId,
                        payment.paymentReference, payment.status, Instant.now());
                String payload = objectMapper.writeValueAsString(event);
                String timestamp = String.valueOf(Instant.now().getEpochSecond());
                callbackClient.post().uri(callbackUrl).header("X-Callback-Timestamp", timestamp)
                        .header("X-Callback-Signature", signature(timestamp, payload)).body(payload).retrieve()
                        .toBodilessEntity();
            }
